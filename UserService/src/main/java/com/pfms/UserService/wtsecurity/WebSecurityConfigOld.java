 ...
 * throws Exception { http.cors().and().csrf().disable().exceptionHandling().
 ...
 * "/h2-console/**", "/localhost:4200/**")
 * .permitAll().anyRequest().authenticated()) .headers(headers ->
 ...
 * config.addAllowedOrigin("http://localhost:8082"); // Add your allowed origins
 * here config.addAllowedHeader("*"); config.addAllowedMethod("*");
